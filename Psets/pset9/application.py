...
def index():
    ...
    stocks = db.execute(
        "SELECT symbol, name, price, SUM(shares) as TotalShares FROM transactions WHERE user_id = ? GROUP BY symbol", session["user_id"])
    ...
    if request.method == "POST":
        ...
        if cash < total_price:
            ...
            db.execute("INSERT INTO transactions (user_id, name, shares, price, type, symbol) VALUES (?, ?, ?, ?, ?, ?)",
                       session["user_id"], item["name"], shares, item["price"], "buy", request.form.get("symbol"))
...
def sell():
    ...
    if request.method == "POST":
       # if not request.form.get("symbol"):
        ...
        current_shares = db.execute("SELECT shares FROM transactions WHERE user_id  = ? AND symbol = ? GROUP BY symbol",
        ...
        db.execute("UPDATE users SET cash = ? WHERE id  = ?", current_cash + total_price, session["user_id"])
        db.execute("INSERT INTO transactions (user_id, name, shares, price, type, symbol) VALUES (?, ?, ?, ?, ?, ?)",
                   session["user_id"], item["name"], - shares, item["price"], "sell", request.form.get("symbol"))
        return redirect("/")
